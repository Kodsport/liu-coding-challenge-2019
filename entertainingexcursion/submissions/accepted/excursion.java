...
public class excursion {
  ...
  public static void main(String[] args) throws IOException {
    ...
    out.println(solve(n, m, xStart, yStart, xFinish, yFinish, xMiddle, yMiddle));
    out.close();
    sc.close();
  }
  ...
  static int solve(int n, int m, int x1, int y1, int x3, int y3, int x2, int y2) {
    boolean mon1 = isMonotonic(x1, x2, x3);
    boolean mon2 = isMonotonic(y1, y2, y3);
    if (mon1 && mon2) {
      return (int)
          ((long) getCountSimplePaths(abs(x1 - x2), abs(y1 - y2))
              * getCountSimplePaths(abs(x2 - x3), abs(y2 - y3))
              % MOD);
    }
    if (x1 == x2 && x2 == x3) {
      long ans = abs(y1 - y3);
    ...
    if (isStrictlyMonotonic(x1, x3, x2) && isStrictlyMonotonic(y1, y3, y2)
        || isStrictlyMonotonic(x3, x1, x2) && isStrictlyMonotonic(y3, y1, y2)) {
      int answer =
          (getLastWay1(n, m, x1, y1, x2, y2, x3, y3) + getLastWay1(m, n, y1, x1, y2, x2, y3, x3))
              % MOD;
      return answer;
    }
  ...
  static int getLastWay1(int n, int m, int x1, int y1, int x2, int y2, int x3, int y3) {
  ...
  static int getLastWay2(int n, int m, int x1, int y1, int x2, int y2, int x3, int y3) {
    int passX = min(abs(x2 - x3), abs(x2 - x1));
    int passY = min(abs(y2 - y3), abs(y2 - y1));
    int longX = max(abs(x2 - x3), abs(x2 - x1));
    int longY = max(abs(y2 - y3), abs(y2 - y1));
    ...
    ret =
        (int)
            ((long) getCountSimplePaths(longX - 1, passY)
                * getCountSimplePaths(passX, longY - passY - 1)
                % MOD);
    for (int it = 0; it < passX; it++) {
      int[][] next = new int[passY + 1][passY + 1];
      for (int i = 1; i <= passY; i++) {
      ...
      for (int i = 0; i < passY; i++) {
        ...
        long way2 = getCountSimplePaths(passX - it - 1, longY - passY - 1);
